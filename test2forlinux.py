...
###########################################################################
my_sender='lukeliuli@163.com' #发件人邮箱账号，为了后面易于维护，所以写成了变量
my_user = 'lukeliuli@qq.com'  # 收件人邮箱账号，为了后面易于维护，所以写成了变量
def mail(mystr):
    ret=True
    try:
        msg=MIMEText(mystr,'plain','utf-8')
        msg['From']=formataddr(["liuli",my_sender])   #括号里的对应发件人邮箱昵称、发件人邮箱账号
        msg['To']=formataddr(["liuli",my_user])   #括号里的对应收件人邮箱昵称、收件人邮箱账号
        msg['Subject']=mystr #邮件的主题，也可以说是标题

        server=smtplib.SMTP("smtp.163.com",25)  #发件人邮箱中的SMTP服务器，端口是25
        server.login(my_sender,"lukeliuli820812")    #括号中对应的是发件人邮箱账号、邮箱密码
...
#    logfile=os.getcwd()+'\\'+time.strftime("%Y-%m-%d", time.localtime())+'.log'
    ...
    for num in range(1, 10):#运行9次
        ...
        chrome_options.add_argument('--no-sandbox') #adding for linux
        driver_url = path1 + '\\chromedriver.exe'
        driver_url = path1+'/chromedriver'
